...
void screen_showmenu(MENU *menu) {
	...
	for(i = 0; i < menu->itemNum; i++, mi++) {
		...
		screen_showitem(SPRX+10, 29+i*14, mi, fg_color);
		if(menu->itemCur == i) 
            print_string(">", fg_color, COLOR_BG, SPRX+10-12, 29+i*14);
	}
}
...
void menuSaveState(void) {
	...
	if (cartridge_IsLoaded()) {
		...
		sprintf(szFile,"%s\\.race-od\\%s",getenv("HOME"),strrchr(gameName,'\\')+1);
#endif
        sprintf(ext,".sta%d",cur_state);
		strcpy(strrchr(szFile, '.'), ext);
